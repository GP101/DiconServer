...
bool KBaseServer::Initialize( DWORD dwParam_ )
{
    ...
    {
        m_kSocketAccepterThread.BindAndListen( m_usDefaultPort, m_serverInitParam.pfnCreateUserCallback );

        const DWORD dwNumWorkingThreads = m_serverInitParam.dwNumIocpThreads;
        // create Iocp working threads and run it.
        _KIocp.Initialize( dwNumWorkingThreads );

        // begin accepter thread.
        m_kSocketAccepterThread.BeginThread();
    }
