...
CONSTRUCTOR KSession::KSession()
    ...
    , m_bAuthKeyReceived( false )
    , m_bCheckHeartBitInterval( true )
{
    m_dwHeartBitTime = GetTickCount();
...
VIRTUAL void KSession::Update(DWORD dwElapsedTime_)
{
    ...
    // check not responding user
    if( m_bCheckHeartBitInterval && ::GetTickCount() - m_dwHeartBitTime > 30000 ) {
        BEGIN_LOG( cwarn, L"Name " << m_wstrName )
            << L" is not responding. tick: "
            << ::GetTickCount() - m_dwHeartBitTime
            << END_LOG;
